...
class KeuanganController extends Controller
{
    public function dataKeuangan(Request $request)
    {
        ...
        // Query base untuk data keuangan pada bulan tersebut, dengan relasi ke pemesanan
        $keuangan = Keuangan::with('pemesanan')
            ->where('bulan', $bulan)
            ->orderBy('tanggal', 'asc')
            ->get();
            ...
            case 'daily':
                ...
                $groupedData = $keuangan->groupBy(function ($item) {
                    ...
                    return [
                        ...
                        'tim_info' => $items->map(function($item) {
                            return $item->pemesanan ? $item->pemesanan->nama_tim : 'Tim tidak tersedia';
                        })->unique()->implode(', ')
                    ];
                });
                break;
                ...
                while ($currentDate <= $end_date) {
                    ...
                    if ($weekItems->count() > 0) {
                        $weekData[$weekStart->format('W')] = [
                            ...
                            'tim_info' => $weekItems->map(function($item) {
                                return $item->pemesanan ? $item->pemesanan->nama_tim : 'Tim tidak tersedia';
                            })->unique()->implode(', ')
                        ];
                    }
    ...
    public function exportKeuanganPDF(Request $request)
    {
        ...
        // Query data keuangan dengan relasi ke pemesanan
        $keuangan = Keuangan::with('pemesanan')
            ->where('bulan', $bulan)
            ->orderBy('tanggal', 'asc')
            ->get();
